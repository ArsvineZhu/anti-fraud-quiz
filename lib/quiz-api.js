...
const { createGameStore, isHostedRuntime } = require('./game-store');
...
function getStore(providedStore, runtime) {
  if (providedStore) return providedStore;
  if (!defaultStore) defaultStore = createGameStore(QUESTIONS, { runtime });
  return defaultStore;
}
...
function readJsonBody(req) {
  ...
  if (requestBody !== undefined) {
    if (requestBody === null) return Promise.resolve({});
    if (typeof requestBody === 'object' && !Buffer.isBuffer(requestBody)) return Promise.resolve(requestBody);
...
function getRoomPayload(state, req, playerId = '') {
  ...
  return {
    ...
    players: getLeaderboard(state, playerId),
  };
}

function getLeaderboard(state, currentPlayerId = '') {
  return rankPlayers(state.players).map((player, index) => ({
    ...
    total: player.assignedQuestionIds.length,
    elapsedMs: player.elapsedMs,
    isMe: Boolean(currentPlayerId && player.id === currentPlayerId),
  }));
}
...
function requireAdmin(req, body, requestUrl, runtime) {
  const configuredPin = String(process.env.ADMIN_PIN || '').trim();
  if (isHostedRuntime(runtime) && !configuredPin) {
...
async function handleApiRequest(req, res, { store, runtime = 'auto' } = {}) {
  const requestUrl = getRequestUrl(req);
  ...
  try {
    if (req.method === 'GET' && requestUrl.pathname === '/api/meta') {
      if (requestUrl.searchParams.get('health') === '1') {
        const healthStore = getStore(store, runtime);
        await healthStore.read();
        sendJson(res, 200, { ok: true, storage: healthStore.kind });
        return true;
      }
    ...
    const activeStore = getStore(store, runtime);
    ...
    if (req.method === 'GET' && requestUrl.pathname === '/api/leaderboard') {
      const state = syncState(await activeStore.read());
      const playerId = requestUrl.searchParams.get('playerId') || '';
      sendJson(res, 200, { players: getLeaderboard(state, playerId) });
      return true;
    }
    ...
    if (req.method === 'GET' && requestUrl.pathname === '/api/admin/room') {
      requireAdmin(req, {}, requestUrl, runtime);
      const state = syncState(await activeStore.read());
    ...
    if (req.method === 'POST' && requestUrl.pathname === '/api/join') {
      ...
      const result = await activeStore.transact((state) => {
        ...
        player.id = `player-${crypto.randomUUID()}`;
      ...
      const result = await activeStore.transact((state) => {
        ...
        return {
          ...
          value: {
            ...
            players: getLeaderboard(updated, playerId),
            tip: question.tip,
          },
        };
      });
    ...
    if (req.method === 'POST' && requestUrl.pathname === '/api/admin/start') {
      const body = await readJsonBody(req);
      requireAdmin(req, body, requestUrl, runtime);
      const result = await activeStore.transact((state) => {
        const next = syncState(state);
